...
_html2canvas.Preload = function( options ) {
  ...
  doc = element.ownerDocument,
  domImages = element.getElementsByTagName('img'), // Fetch img of the present element only
  ...
  function start(){
    Util.log("html2canvas: start: img: " + images.numLoaded + " / " + images.numTotal + " (failed: " + images.numFailed + ")");
    if (!images.firstRun && images.numLoaded >= images.numTotal){
      Util.log("Finished loading img: # " + images.numTotal + " (failed: " + images.numFailed + ")");
  ...
  // TODO modify proxy to serve img with CORS enabled, where available
  ...
  Util.log('html2canvas: Preload starts: finding background-img');
  images.firstRun = true;

  getImages(element);

  Util.log('html2canvas: Preload: Finding img');
  // load <img> img
...
window.html2canvas = function(elements, opts) {
  ...
  options = {
    ...
    useCORS: false, // try to load img as CORS (where available), before falling back to proxy
    allowTaint: false, // whether to allow img to taint the canvas, won't need proxy if set to true
    ...
    width: null,
    height: null,
    taintTest: true, // do a taint test with all img before applying to canvas
    renderer: "Canvas"
  };
  ...
  // for pages without img, we still want this to be async, i.e. return methods before executing
